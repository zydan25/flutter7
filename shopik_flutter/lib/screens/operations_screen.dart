...
class _OperationsScreenState extends State<OperationsScreen> {
  ...
  Future<void> _loadOperations() async {
    ...
    // Fallback to controller operations or verified baseline matching screenshot 3
    final app = context.read<AppController>();
    if (app.operations.isNotEmpty) {
      setState(() {
        _operations = List.from(app.operations);
        _loading = false;
      });
    } else {
      // Default verified dataset directly matching Screenshot 3
      setState(() {
        _operations = [
          {
